...
namespace NSI.Core.Validation.Rules {
  /// <summary>
  /// Asynchronously validates a nested object property using its registered validator.
  /// </summary>
  ...
  ///   <item><description>Apply validation asynchronously to the nested object</description></item>
  ...
  /// var addressRule = new NestedAsyncValidationRule&lt;User, Address&gt;(u => u.Address);
  /// var errors = await addressRule.ValidateAsync(user, validationContext);
  /// </code>
  /// </para>
  /// </remarks>
  public sealed class NestedAsyncValidationRule<T, TProperty>: IAsyncValidationRule<T>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="NestedAsyncValidationRule{T,TProperty}"/> class.
    /// </summary>
    ...
    /// injection container and applies it asynchronously to the property value.
    /// </para>
    /// <para>
    /// The method will:
    /// <list type="bullet">
    ...
    public async Task<IEnumerable<IValidationError>> ValidateAsync(
      ...
      var result = await validator.ValidateAsync(propertyValue, context, cancellationToken);

      // Prefix property names with parent property name
      return (IEnumerable<IValidationError>)result.Errors.Select(error => {
