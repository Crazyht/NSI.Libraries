...
public sealed class ProjectionSpecificationTests {
  ...
  public void Selector_Wraps_And_Projects_InMemory() {
    var data = new[] {
      new Person(1, "Ada", "Lovelace"),
      new Person(2, "Alan", "Turing")
    };

    IProjectionSpecification<Person, PersonDto> spec =
      new ProjectionSpecification<Person, PersonDto>(
        p => new PersonDto(p.Id, p.First + " " + p.Last)
      );
  ...
  public void Select_NullArguments_Throw() {
    ...
    Assert.Throws<ArgumentNullException>(() =>
      ProjectionExtensions.Select<Person, PersonDto>(
        (IQueryable<Person>)null!,
        spec
      )
    );
